...
public class Decryption extends JFrame implements ActionListener {
    ...
    public Decryption() {
        super("Декодирование стегонографического сообщения в изображении");
        assembleInterface();
    ...
    private void assembleInterface() {
        ...
        p.setBorder(BorderFactory.createTitledBorder("Декодированное сообщение"));
        message.setEditable(false);
        this.getContentPane().add(p, BorderLayout.SOUTH);

        imagePane.setBorder(BorderFactory.createTitledBorder("Изображение со стеганографией"));
    ...
    private java.io.File showFileDialog(boolean open) {
        JFileChooser fc = new JFileChooser("Открыть изображение");
    ...
    private void decodeMessage() {
        if (image == null) {
            JOptionPane.showMessageDialog(null, "сначала откройте фотографию");
            return;
        }
        int len = extractLengthMessage(image);
        byte b[] = new byte[len];
        for (int i = 0; i < len; i++)
    ...
    private int extractLengthMessage(BufferedImage img) {
